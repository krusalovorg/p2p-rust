...
impl SignalServer {
    ...
    async fn handle_connection(self: Arc<Self>, socket: TcpStream) {
        ...
        loop {
            ...
            for item in peers_guard.iter() {
                let p_locked = item.lock().await;
                if &p_locked.info == peer_info {
    ...
    async fn send_peer_info(peer: Arc<Mutex<Peer>>, peer_info: String) {
        ...
        let socket = peer.lock().await.socket.clone(); // Не блокируйте на этой строке
    ...
    async fn wait_for_peers(self: Arc<Self>) {
        loop {
            ...
            for peer in peers_guard.iter() {
                let p_locked = peer.lock().await;
                if p_locked.wait_connection {
                    drop(p_locked);
            ...
            if have_wait_connection_peers % 2 == 0 && have_wait_connection_peers > 0 {
                ...
                SignalServer::send_peer_info(
                    first_peer.clone(),
                    second_peer.lock().await.info.clone(),
                )
                .await;
                SignalServer::send_peer_info(
                    second_peer.clone(),
                    first_peer.lock().await.info.clone(),
                )
                .await;

                let mut first_peer = first_peer.lock().await;
                first_peer.wait_connection = false;

                let mut second_peer = second_peer.lock().await;
                second_peer.wait_connection = false;
            }
            drop(peers_guard);
